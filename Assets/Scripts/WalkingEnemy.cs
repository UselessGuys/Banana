...
internal class WalkingEnemy : MonoBehaviour, IEnemy
{
   ...
   void OnTriggerEnter2D(Collider2D collider)
    {
        if (collider.gameObject.CompareTag("Player"))
            collider.gameObject.GetComponent<CharacterStats>().Health -= 10;
    }
    ...
    private void CheckObstacles()
    {
        for (int i = 0; i < rayCount + 1; i++)
        {
            ...
            if ((hit.collider != null) && (hit.collider.CompareTag("Ground")) 
                && ((hit.collider.transform.position.y + hit.collider.bounds.size.y / 2) - hit.point.y) > DynamicObject.MaxHorisontalLadder)
            {
